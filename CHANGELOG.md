...
- **Six new themes, all original.** No ports this round — each one is tide's own: **abyss** (deep-ocean dark, bioluminescent cyan over near-black blue), **blackwater** (true #000000 for OLED panels, hairline borders, thin progress + slider, thumbnails off — nothing glows that doesn't have to), **golden hour** (warm sunset light theme — sand, espresso, burnt orange + dusty rose, polaroid album art), **seaglass** (the smoked-glass one, see below), **storm** (slate greys, hard edges, IBM Plex Mono in UPPERCASE, `// ` list markers, one electric-yellow lightning accent, oscilloscope), and **undertow** (moody indigo depths with a single blood-red accent and a red wedge for volume). Every theme picks its own visualizer, controls, text case and widget variants — they're personalities, not palettes.
- **seaglass is genuinely translucent.** New `[layout] window_translucent` theme capability: the window gets a real alpha channel (`#AARRGGBB` background tokens), so the desktop shows through the smoked slate-teal glass — amber and lime accents sampled straight from the reference artwork. The adaptive backdrop paints alpha-aware so the tint never doubles up. KDE users who want the full frosted look can add a KWin blur rule for the tide window (e.g. the `kwin-effects-forceblur` AUR package); without it you get smoked glass, which already slaps.
